...
/obj/item/rogueweapon/attack_self(mob/living/user)
	if(!charge)
		return ..()
...
/obj/item/rogueweapon/attack(mob/living/target, mob/living/user)
	. = ..()
	if(!.)
		return FALSE
...
/obj/item/rogueweapon/proc/HandleCharge(added_charge, mob/target)
	if(target)
		if(target.stat == DEAD) // lets not give them charge for beating up a dead body
			return FALSE
	...
	if(charge_amount < charge_cap)
		...
		new /obj/effect/temp_visual/damage_effect/bleed(get_turf(src)) //bleed instead of charge cuz these r supposed to go for vampire weapons

/// Lets people refund their charge if the allow_ability_cancel var is set to TRUE
/obj/item/rogueweapon/proc/CancelCharge(mob/user)
...
/// default does nothing lol
/obj/item/rogueweapon/proc/ChargeAttack(mob/living/target, mob/living/user)
	sleep(0.2 SECONDS)
